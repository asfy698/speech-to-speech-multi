...
class Qwen3TTSHandler(BaseHandler[TTSIn, TTSOut]):
    ...
    def setup(
        ...
        self.output_speed = float(
            self.gen_kwargs.pop("output_speed", os.getenv("QWEN3_TTS_OUTPUT_SPEED", "0.85"))
        )
    ...
    def _apply_output_speed(self, audio: np.ndarray) -> np.ndarray:
        speed = float(self.output_speed)
        ...
        down = 1000
        gcd = np.gcd(up, down)
        return resample_poly(audio, up=up // gcd, down=down // gcd)
    ...
    def _stream(self, gen: Any, label: str) -> Iterator[bytes | np.ndarray]:
        ...
        for item in gen:
            ...
            audio_chunk = self._apply_output_speed(audio_chunk)
