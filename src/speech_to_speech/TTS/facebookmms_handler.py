...
class FacebookMMSTTSHandler(BaseHandler[TTSIn, TTSOut]):
    ...
    def process(self, tts_input: TTSIn) -> Iterator[TTSOut]:
        ...
        peak = float(np.max(np.abs(audio_resampled))) if audio_resampled.size else 0.0
        if peak > 0.0 and peak < 0.8:
            audio_resampled = audio_resampled * (0.95 / peak)
            logger.debug("Normalized audio peak from %.4f to 0.95", peak)
