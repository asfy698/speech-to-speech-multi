...
class AudioHandler(RealtimeBaseHandler):
    ...
    def handle_audio_append(self, conn_id: str, event: InputAudioBufferAppendEvent) -> list[bytes]:
        ...
        if chunks:
            ...
            self._service.add_stt_bytes(conn_id, sum(len(chunk) for chunk in chunks))
        return chunks
    ...
    def handle_audio_commit(self, conn_id: str) -> RealtimeErrorEvent | None:
        ...
        self._service.mark_audio_sent_to_stt(conn_id)
    ...
    def on_speech_started(self, conn_id: str, event: SpeechStartedEvent) -> list[ServerEvent]:
        ...
        st = self._state(conn_id)
        if not event.reopened or self._service._current_loop(conn_id) is None:
            self._service.start_loop(conn_id)
        self._service._set_phase(conn_id, "stt", "speech detected")
    ...
    def on_speech_stopped(self, conn_id: str, event: SpeechStoppedEvent) -> list[ServerEvent]:
        ...
        self._service._set_phase(conn_id, "stt", "speech stopped, waiting for transcription")
        return [
            InputAudioBufferSpeechStoppedEvent(
    ...
    def encode_audio_chunk(self, conn_id: str, audio: bytes) -> list[ServerEvent]:
        ...
        response = self._service.response
        st = self._state(conn_id)
        self._service._set_phase(conn_id, "tts", "streaming assistant audio")
        if st.current_loop is not None and st.current_loop.tts.started_at_s is None:
            self._service.start_tts(conn_id, st.tts_model_name)
