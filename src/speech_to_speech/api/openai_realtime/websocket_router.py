...
def _infer_pipeline_models(unit: PipelineUnit) -> dict[str, str | None]:
    """Extract STT/TTS model names from the pipeline handlers attached to a unit."""
    ...
    for handler in unit.handlers:
        ...
        model_name = getattr(handler, "model_name", None)
        if llm_model_name is None and handler_module.startswith("speech_to_speech.LLM"):
            llm_model_name = None if model_name is None else str(model_name)
        if stt_model_name is None and handler_module.startswith("speech_to_speech.STT"):
            stt_model_name = None if model_name is None else str(model_name)
        elif tts_model_name is None and handler_module.startswith("speech_to_speech.TTS"):
            tts_model_name = None if model_name is None else str(model_name)
    return {
...
def _client_gateway_token(ws: WebSocket) -> str | None:
    token = ws.query_params.get("token")
    if token:
        return token
    token = ws.headers.get("x-s2s-token")
    if token:
        return token
...
def create_app(
    ...
    async def realtime_endpoint(ws: WebSocket) -> None:
        if gateway_token is not None:
            provided_token = _client_gateway_token(ws)
            if provided_token != gateway_token:
                logger.warning("Rejected connection: invalid gateway token")
                await ws.close(code=1008, reason="Invalid gateway token")
                return
        ...
        unit.service.register_pipeline_models(session_id, **_infer_pipeline_models(unit))
    ...
    async def stats_endpoint() -> dict[str, Any]:
        """Return a compact live diagnostics snapshot for the realtime service pool."""
        total: dict[str, Any] = {}
        connections: list[dict[str, Any]] = []
        for unit in pool:
            diagnostics = unit.service.get_diagnostics()
                            ...
                            if isinstance(sub, dict):
                                for sub_k, sub_v in v.items():
                                    if isinstance(sub_v, (int, float)):
                                        sub[sub_k] = sub.get(sub_k, 0) + sub_v
                connections.extend(diagnostics.get("connections", []))
        should_listen = None
        if pool and pool[0].service.should_listen is not None:
            should_listen = pool[0].service.should_listen.is_set()
        return {
            "usage": total,
            "pool_size": len(pool),
            "active_connections": len(connections),
            "connections": connections,
            "current_phase": connections[0]["phase"] if connections else "idle",
                ...
                try:
                    ...
                    if _is_audio_done(audio_chunk):
                        ...
                        if audio_generation is not None and unit.cancel_scope.is_stale(audio_generation):
                            ...
                            continue
                        if session_id:
                            unit.service.finish_tts(session_id)
                        await _drain_pending_response_events(ws, unit, session_id)
                        if ws is not None and session_id:
                            await _send_events(ws, unit.service.finish_response(session_id))
                        if session_id:
