...
from speech_to_speech.robot.esp32_motors import ESP32MotorController, build_motor_tools
...
class ListenAndPlayRealtimeArguments:
    ...
    language: str = field(
        default="en",
        metadata={
            "help": "Assistant language for replies. Choose en, ta, or hi.",
        },
    )
...
def _build_session_update(args: ListenAndPlayRealtimeArguments) -> dict:
    ...
    language_prompt = language_prompts.get(args.language, "Always respond in English.")
    ...
    session = {
        ...
        "tools": build_motor_tools(),
    }
    if args.instructions:
        session["instructions"] = (
            f"{args.instructions}\n\n"
            f"{language_prompt}\n"
            "ESP32 robot tools are available for motor control. "
            "Use move_forward, move_backward, and stop_robot when the user asks the robot to move."
        )
    else:
        session["instructions"] = (
            f"{language_prompt}\n"
            "You can control the ESP32 robot using move_forward, move_backward, and stop_robot. "
            "Use the tool that matches the user's request."
        )
...
async def listen_and_play_realtime(args: ListenAndPlayRealtimeArguments) -> None:
    ...
    esp32 = ESP32MotorController(os.getenv("ESP32_BASE_URL") or os.getenv("ESP32_IP"))
    audio_gain = float(os.getenv("AUDIO_GAIN", "1.0"))

    def resolve_audio_device(kind: str, requested: Optional[int]) -> Optional[int]:
        ...
        if kind == "input":
            for idx, device in enumerate(devices):
                ...
                if device.get("max_input_channels", 0) > 0 and any(pref in name for pref in preferred_names):
                    candidates.append(idx)
        else:
            for idx, device in enumerate(devices):
                name = str(device.get("name", "")).lower()
                if device.get("max_output_channels", 0) > 0 and any(pref in name for pref in preferred_names):
                    candidates.append(idx)
    ...
    input_device = resolve_audio_device("input", args.input_device)
    output_device = resolve_audio_device("output", args.output_device)

    try:
        print(f"Audio input device: {input_device} -> {sd.query_devices(input_device)['name'] if input_device is not None else 'auto'}", flush=True)
    except Exception:
        print(f"Audio input device: {input_device}", flush=True)
    try:
        print(f"Audio output device: {output_device} -> {sd.query_devices(output_device)['name'] if output_device is not None else 'auto'}", flush=True)
    except Exception:
        print(f"Audio output device: {output_device}", flush=True)
    ...
    async def receive_events(conn):
        ...
        while not stop_event.is_set():
            ...
            if event.type == "session.created":
                ...
                print(f"TOOL: {event.name} call_id={event.call_id} arguments={event.arguments}", flush=True)
                result = esp32.execute(event.name)
                if not result.get("ok"):
                    print(f"ESP32 ERROR: {result}", flush=True)
                else:
                    print(f"ESP32 OK: {result['status_code']} {result['url']}", flush=True)
                await conn.send(
                    {
...
def main() -> None:
    ...
    parser.add_argumet("--base-url", default=defaults.base_url)
    ...
    parser.add_argumennt("--recv-rate", type=int, default=defaults.recv_rate)
    ...
    parser.add_argument(
        ...
        choices=["en", "ta", "hi"],
        help="Assistant language for replies: en, ta, or hi.",
    )
